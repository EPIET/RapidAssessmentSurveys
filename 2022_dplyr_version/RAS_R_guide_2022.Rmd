...
table451 <- vaccine %>% 
  ...
  sitrep::tab_survey(vacful,           # The variable to calculate proportions on
                     ...
                     method = "xlogit",# Log transformation of mean
                     wide = FALSE,     # Long format better for a lot of strata
                     deff = TRUE,      # Include the design effect in the output
...
Because most of this code is repetitive we will use the `map()` function from the 
`purrr` package to iterate (repeat) code for each of this variables. This may 
seem scary at first, but try to remember that we are just using the functions 
from previous sessions and running in multiple times, once for each variable. 
`purrr` will store the output in a list object, but using `bind_rows()` we easily
and simply combine these in to a single data frame. 
...
group and then by urban / rural area of residence using the `tab_survey()` function 
from the `sitrep` package directly. 
...
restab1 <- purrr::map(
  ...
  .f = function(x) {
    ...
    vce_design %>% 
      ...
      # note that here we dont need curly brackets (because we want it to be a character)
      rename(stratum = x) %>%
...
For the next set of variables to tabulate, we do not require stratification. So 
instead of iterating over the stratifying variable, we can iterate over each of 
the vaccines to get an overall estimate for each.
...
Next, we can look at vaccine completeness stratified by minority group and
urban/rural residence. We repeat the above process to run each of the variables
through the strata argument of the `tab_survey()` function from `sitrep`. 
...
Finally we can run the same for vaccination timeliness. Note that we could have
saved some repeating code by nesting `map()` arguments, to iterate over both 
vaccines and strata groups. For simplicity, we have not done this, but if you have
extra time - try this out. 
...
Lastly, we can combine all the tables together and tidy up the decimals and add 
percentages to make the results easier to read, as before.
