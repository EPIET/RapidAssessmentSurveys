...
# Convert to flextable:
ssizeprop_pub <- flextable::qflextable(region_frame)
...
`survey::svyciprop()` function.

If you type `?survey::svyciprop` in the console to look at the help page
for this function, you will find further details on the different
methods available. The default method is `logit` but we have used the
`xlogit` method here as it is somewhat closer to the equivalent results
that you would get if performing this analysis in STATA.
...
Refer to the blank tables in the main case study guide (4.5.1 and 4.5.2)
to decide how you will structure your analysis, using the techniques
already demonstrated in the previous tasks.
...
just using the functions from previous sessions and running in multiple
times, once for each variable. `purrr` will store the output in a list
object, but using `bind_rows()` we easily and simply combine these in to
a single data frame.
...
group and then by urban / rural area of residence using the
`tab_survey()` function from the `sitrep` package, and `purr::map()` to
loop over the two stratifiers:
...
Finally we can run the same for vaccination timeliness. Note that we
could have saved some repeating code by nesting `map()` arguments, to
iterate over both vaccines and strata groups. For simplicity, we have
not done this, but if you have extra time - try this out.
...
In this session you will be working with a new data set, which combines
the previous data with questionnaire responses from the parents of
children recruited to the study. The questionnaire explored parental
beliefs about vaccination, and the researchers are interested in finding
out if there is any correlation between parental beliefs and the
vaccination status of their child.

In the first few tasks, you will create descriptive summaries of the
different parental characteristics.

In subsequent tasks you will then perform some analysis to determine
which parental characteristics have a statistically significant
association having a fully vaccinated child.

In the last part of this session, you will complete your analysis to
include some multivariable modelling.

As before, please refer to the following appendices for the data
dictionary:
...
### Task 5.1 - vaccination coverage by parental characteristics

**Background:**

In this task you will be completing the blank fields (highlighted in
yellow) in `table 5.1` presented in the main case study guide. To do
this, you will need to estimate proportions in the population
(numerically and as a percentage, including 95% confidence intervals),
allowing for weights, stratification and clustering as before. This time
you will look at some parental characteristics listed in table 5.1.
...
4.  Estimate the proportions for each of the parental characteristics
    listed in table 5.1, using a survey design that takes into account
    the weights, clustering and strata that were used during the
    sampling.

The parental characteristics to estimate proportions for from table 5.1
are as follows:

| Variable description       | Variable name | Comments                                      |
|---------------------|------------------|---------------------------------|
| Minority group             | `minority`    | Convert to factor and add factor level labels |
| Mother's age at childbirth | `mage`        | Create age groups: \<25, 25-29, \>=30         |
| Number of other siblings   | `osib1`       | Divide into categories: \>=3, 1-2, 0          |

: Table 5.1 parental characteristics to estimate proportions for

**R coding tips:**

As a first step, we will import the data `vaccine5.dta` which you can
find in the `data` folder, as before.

We will then create new variables for mother's age group and categories
for the number of siblings, as shown above, using `dplyr::case_when()`
to assign the categories. We will also convert the `minority` variable
to a labelled factor, as we did in task 4.1.

We will then create the survey design, including weights, clustering and
strata, with the `srvyr` package, as before. You will need the following
variables to do this:

-   `weight` : contains sampling weights as calculated earlier in the
    case study;

-   `strata` : contains the 11 strata (urban and rural areas within the
    6 regions);

-   `school` : contains the cluster ID for each school class that was
    included in the study.

*A - Data import and preparation*

Here we will convert `minority` to a labelled factor, using the base R
function `factor()` .

Then we will use `dplyr::case_when()` to create categorical variables
for age group and number of siblings, respectively, from the numeric
columns `mage` and `osib1`.

Finally we will convert the two new categorical variables to an ordered
...
vaccine <- rio::import(here::here("data", "vaccine5.dta"))  %>% 
  ...
  # Create new variable containing mother's age group:
  ...
  # Convert to a factor and put the agegroups in ascending order:
  mutate(magegroup = fct_relevel(magegroup, 
                                 "Under 25", 
                                 "25 - 29", 
                                 "30 or over")) %>% 
  
  # Create new variable containing categories for number of siblings:
  ...
  mutate(nsibcat = fct_relevel(nsibcat, 
                               "3 or more", 
                               "1 - 2", 
                               "No siblings")) 
...
In this code chunk, we will create the survey design from the data set,
using `srvyr::as_survey_design()` as before.
...
2.  In the `sitrep::tab_survey()` function we will set the argument
    ...
    proportions for all of the factor levels in each variable. This is
    in contrast to when we were calculating vaccine coverage in the
    previous section; where we set `keep = c(1)` to indicate that we
    only wanted to calculate the proportion of vaccinated individuals
    (in all the vaccine variables, vaccinated = 1, not vaccinated = 0).
...
restabpub <- restab452a %>% 
  ...
  # Now we can convert the estimates and 95% CI to percentages:
  mutate(across(contains("proportion"), ~ percent(.x, accuracy = 0.01))) %>%
  ...
  flextable::border_inner_h(border = fp_border(
    color = "black", 
    style = "solid", 
    width = 1), 
    part = "body")
...
### Task 5.2 - Bivariate statistics and crude associations with vaccination
...
associations between these factors and vaccination coverage. The
objective is to complete table 5.2 in the main case study guide. This
includes calculation of bivariate statistics (vaccine coverage
stratified by different parental characteristics and attitudes) as well
as crude prevalence ratios. If you have time, you can also perform
univariate analysis and calculate crude odds ratios.
...
1.  Tabulate bivariate statistics (create a 2x2 table) to describe
    ...
    not. Use the same survey design as for previous sections, taking
    into account weights (`weight`), clustering (`school`) and strata
    (`strata`).
...
2.  Calculate crude measures of association, with corresponding 95%
    ...
    including the `school` variable as a random effect.
...
3.  If you have time, complete the optional exercise and calculate crude
    ...
    listed in table 5.2 as explanatory variables. As with prevalence
    ratios, include `school` as a random effect. Note that odds ratios
    overestimate the risk ratio when an outcome is common, or above
    approximately 10% prevalence. For this reason, odds ratios may not
    be an appropriate measure of association for many of the explanatory
    variables in this data set.

For convenience, the explanatory variables to analyse are listed in the
table below. Note that some will require labelling to make the output
more human readable.

| Description                                | Variable     | Comments                     |
|------------------------------|---------------------|---------------------|
| Minority group                             | `minority`   | Already a labeled factor     |
| Mother's age at childbirth                 | `magegroup`  | Reverse order of levels      |
| Father's education                         | `educf`      | Requires labelling (category) |
| Number of siblings                         | `nsibcat`    | Already a labeled factor     |
| Balanced positive opinion on vaccination   | `a1posyn`    | Requires labelling (binary)   |
| Balanced positive opinion on vaccination   | `a1posg`     | Requires labelling (category) |
| Uncritical positive opinion on vaccination | `uncritical` | Requires labelling (binary)   |
| Vaccination is necessary for my child      | `a1x1g`      | Requires labelling (binary)   |
| Vaccines may harm my child                 | `a1x2g`      | Requires labelling (binary)   |
| Natural disease preferable to vaccines     | `a1x5g`      | Requires labelling (binary)   |
| Long distance to immunization site         | `a3x1g`      | Requires labelling (binary)   |
| Inconvenient opening hours                 | `a3x2g`      | Requires labelling (binary)   |
| Cost of vaccines                           | `a3x5g`      | Requires labelling (binary)   |

**R coding tips:**

You will first need to convert all the explanatory variables to labelled
factors. A few of these have already been converted to factors in the
previous section; to quickly deal with the remainder, we can use
`dplyr::across()` to loop through the list of binary (0,1) variables and
label them as (no, yes).

Next, you can calculate the summary counts *N* for each explanatory
variable, by cross-tabulating `vacful` with the variables of interest.

You can then calculate full vaccine coverage for each explanatory
variable by looping through them with `purrr::map()` and using
`sitrep::tab_survey()` as before.

*NB:* You can use the same survey design, but it is advisable to run the
code to create it again after updating or making changes to any
variables; if not, attempts to use the old survey design on a revised
data set will result in an error, indicating that the mutated columns
cannot be found.

The Poisson model can be created using the `glmmML::glmmML()` function,
which creates generalized linear mixed effects Poisson (or binomial)
regression models, with clustering. You will need to specify the
following arguments:

-   *formula* in the format `outcome ~ dependent` variable (e.g.
    `vacful ~ minority`)

-   *family*: select the type of model to run, e.g. `poisson`

-   *data*: name the data set, e.g. `vaccine`

-   *cluster*: name the clustering variable, e.g. `school`

To obtain prevalence ratios and 95% confidence intervals from the model
results, we will apply the following formulas to the model output:

1.  $PR = exp(coeff)$ will calculate the prevalence ratios;

2.  $CI = (coeff + / - 1.96 \times se)$ to calculate 95% confidence
    intervals, where:
...
Finally, if you have time to complete the optional exercise, you can
repeat the `glmmML::glmmML()` model with the same parameters, but this
time using `family = binomial` .

*A - preparing the data:*

First, we will convert the variables to labelled, ordered factors, with
the first level of each factor representing the baseline (intercept)
with which we will compare the other levels. For binary variables, this
will be `0 = no` . For the non-binary categorical variables, this will
normally be the smallest numeric category, e.g. `no siblings` for number
of siblings, or the most general category, e.g. `General population` for
minority group.

```{r s52_prep_data}

############################################
# 01. Convert variables to labelled factors
###########################################

# Call up the data set:
vaccine <- vaccine %>% 
  
  # Reverse the factor level order of nsibcat so it starts with "no siblings":
  mutate(nsibcat = fct_relevel(nsibcat, "No siblings", "1 - 2", "3 or more")) %>% 
  ...
  # Create a labelled factor from a1posg (balanced positive opinion, categories):
  mutate(a1posg = factor(a1posg, 
                         levels = c(0, 1, 2), 
                         labels = c("No positive opinion", 
                                    "Moderate positive opinion", 
                                    "High positive opinion")))

###################################################
# 02. Convert binary variables to labelled factors
##################################################

# Now we can create a list of binary variables (no / yes) to label:
...
# Next, we can use mutate(across()) to convert them to labelled factors:
vaccine <- vaccine %>%  
  
  # Mutate across:
  mutate(across(
    .cols = all_of(binary_vars),    # Columns in the list binary_vars
    .fns = ~factor(.x,              # Convert to a factor           
      levels = c(0, 1),             # Stipulate levels from existing values
      labels = c("no", "yes"))))    # Label levels as no and yes


###################################
# 03. Update survey design
###################################

# Recreate the survey design to include the mutated variables:
vce_design <- vaccine %>% 
...
#######################################################
# 04. Create labels for all variables to be tabulated
######################################################

# Create a list of all variables to analyse:
allvars <- c("minority", 
             "magegroup", 
             "educf",
             "nsibcat", 
...
*B - Counts of fully vaccinated children:*

In this section we will create a table of counts for vaccinated children
by each of the variables listed in `allvars`. We can use the `sitrep`
function `tab_linelist()` to generate the counts for each
factor level in each variable from `allvars`.

We will also add human readable question labels to the variable names, which will make the final results table easier to understand.

We will then append the columns containing crude vaccination coverage,
prevalence ratios and odds ratios to this base table.
...
*C - Vaccination coverage stratified by parental characteristics and beliefs:*

Here we will repeat the same procedure as demonstrated in previous
sections to estimate full vaccination coverage (`vacful`) taking into
account weights, strata and clustering in `vce_design` and stratifying
the results by each of the variables in `allvars`. We will loop through
each of the variables using `purrr::map()` as before. We will then
append the results to the table of counts we created in the previous
section, using `bind_cols()` .
...
restab_vce <- purrr::map(
  ...
  .f = function(x) {
    ...
    vce_design %>% 
      sitrep::tab_survey(vacful,                # VCE will be calculated for vacful
                         strata = all_of({x}),  # each variable in allvars is a stratum
                         keep = c(1),           # we are only interested in vacful == 1
                         na.rm = TRUE,
                         method = "xlogit",
                         wide = FALSE,     
                         deff = TRUE,      
      ...
      # Rename factor level column to stratum:
...
# Convert binding variable to factor:
restab52b <- restab52a %>% 
...
*D - Crude prevalence ratios for vaccination in association with parental characteristics and beliefs:*

In this section you will create and run a Poisson regression model, tidy
up the results, put them in a table and loop the variables in `allvar`
through the whole process, as before. As weights are not necessarily as
relevant for this process (see discussion in main case study guide), we
will just account for clustering by school class.

It is advisable to first run the model on a single variable, to become
familiar with the inputs and outputs. An example for `vacful ~ minority`
is included below. The overall results can be reviewed with the
`summary()` command, while specific elements are stored as lists.
Because these are raw results, you will need to extract the list of
`coefficients` from the model output, which can be done in the same way
as referencing columns in a data set, e.g. `mymodel$coefficients` ,
where `mymodel` is the name you have given to store the model output.
These need to be exponentiated to give the prevalence ratios. 

In addition, you will also need to extract the standard errors, which are stored in `coef.sd`. While there are helper functions (in the `broom` package) that would do this heavy lifting for you with a standard `glm()`, this does not yet exist for `glmmML()` so we need to do it ourselves.

Note that a robust variance estimate is already made with the methods used in the `glmmML` package, so it does not need to be specified. Running this model as a standard `glm()` (available in base R) gives less robust estimates.

The `glmmML()` function has a special argument for clustering, called `cluster`. According to the study design, clustering was performed at the level of school classes (`school`). However, the school class IDs are numeric and therefore need to be converted to a factor before being passed to the model (otherwise it would be treated like a continuous variable).

```{r s52_poisson_model}

#######################################
# 01. Becoming familiar with the model
#######################################

# Before running the model on the list of variables, try running it on one:
pmodel1 <- glmmML::glmmML(formula = vacful ~ minority,  # outcome ~ explanatory 
                          family = poisson,             # poisson or binomial
                          cluster = factor(school),     # cluster variable
                          data = vaccine)               # Data set to use

# Now look at the summary output:
summary(pmodel1)

# And pull out just the coefficients:
pmodel1$coefficients

# Or just the standard errors:
pmodel1$coef.sd

# To calculate prevalence ratios, exponentiate the coefficients:
# Note this is a named list - the names are made up of the variable + respnse
pr <- exp(pmodel1$coefficients)

# To calculate lower 95% confidence intervals:
lci95 <- exp(pmodel1$coefficients - 1.96 * pmodel1$coef.sd)

# To calculate upper 95% confidence intervals:
uci95 <- exp(pmodel1$coefficients + 1.96 * pmodel1$coef.sd)

# To tabulate results:
poisondf <- tibble(value = names(pmodel1$coefficients), # extract names too
                   pr = exp(pmodel1$coefficients), 
                   lci95 = exp(pmodel1$coefficients - 1.96 * pmodel1$coef.sd), 
                   uci95 = exp(pmodel1$coefficients + 1.96 * pmodel1$coef.sd))
...
Next, we can create a function that will perform the same tasks, put the
model results into a table and tidy up the output:

```{r s52_poison_tabulator_function}

# Create function to loop through variables:
poison_tabulator <- function(predictor) {
  
  # Construct the model formula:
  pm_formula = paste0("vacful ~ ", predictor)
  
  # Construct the model:
  modres = glmmML::glmmML(formula = as.formula(pm_formula), 
                          family = poisson,
                          cluster = factor(school), 
                          data = vaccine) 
  
  # Construct results table:
  modtab = tibble(
    
    # Get the variable and response labels:
    varids = names(modres$coefficients), 
    
    # Calculate and add the prevalence ratios:
    pr = exp(modres$coefficients), 
    
    # Calculate and add the lower 95% confidence intervals:
    lci95 = exp(modres$coefficients - 1.96 * modres$coef.sd), 
    
    # Calculate and add the upper 95% confidence intervals:
    uci95 = exp(modres$coefficients + 1.96 * modres$coef.sd)) %>% 
    
    # Filter out the intercepts:
    filter(varids != "(Intercept)") %>% 
    
    # Separate variable name and value label into 2 columns:
    tidyr::separate(col = varids, 
                    into = c("variable", "value"), 
                    sep = predictor) %>% 
    
    # Fill variable column with the name of the variable that was modelled:
    mutate(variable = predictor) %>% 
    
    # Tidy up numbers (round to 2 decimal places):
    mutate(across(c(pr, lci95, uci95), ~ round(.x, digits = 2))) %>% 
...
Because the function only has one input (the predictor or explanatory
variable that you want to model), we can simply pipe the list of
variables into the function to run it, and then `bind_rows()` to combine
all the results into one table:

```{r s52_poison_results_table}

########################################
# 02. Looping allvars through the model
########################################
...
restab_pois <- allvars %>% 
  ...
  purrr::map(poison_tabulator) %>% 
  
  # Bind the results into a single table:
  bind_rows() %>% 
  
  # Combine PR and 95% CI:
  mutate(pr95 = paste0(pr,
                       " (",
                       lci95,
                       " - ",
                       uci95,
  ...
  # Select columns to drop for the final table:
  select(-c(pr, lci95, uci95))
  

```

We can now merge these results with the table containing counts and vaccine coverage, using the`variable` and `value` columns to ensure the matches are unique. Note that we removed the intercepts - in table 5.2, these have been encoded as 1, since they were used as reference levels.  When the tables are merged, the reference levels will present as missing values (`NA`) which we can then recode to `ref` with the `dplyr::case_when()` function.
...
restab52c <- restab52b %>% 
  ...
  left_join(y = dplyr::select(.data = restab_pois,
  ...
  # Otherwise leave the rest of the values as they are:
   mutate(pr95 = case_when(is.na(pr95) ~ "ref", TRUE ~ pr95))

```

If you have time, you are welcome to try the optional exercise, which is
to calculate odds ratios instead of prevalence rates, keeping all other
parameters the same as above and comparing the results.

The easiest way to do this is to change the `family` argument in the
`glmmML::glmmML()` function from poisson to `binomial` and then repeat
the rest of the steps, as above. 

Below is the modified tabulator function to calculate crude odds ratios:
...
or_tabulator <- function(predictor) {
  ...
  modtab = tibble(
    ...
    # Tidy up numbers (round to 2 decimal places):
    mutate(across(c(or, lci95, uci95), ~ round(.x, digits = 2))) %>% 
...
Remember to give the output a different name, so that it does not get over-written.  Below we demonstrate how to use the `or_tabulator` function for one variable (minority) and then for a group of variables by supplying them to the function as a list.  As for prevalence ratios, the function tidies up the raw model output into a results table, calculates 95% confidence intervals from the standard errors and exponentiates the results. 

```{r s52_or_examples}

# Example with a single variable:
tabres_or <- or_tabulator("minority")

##############################################
# 02. Calculate crude odds ratios for allvars
##############################################

# Apply the model to the list of variables called 'allvars':
restab_or <- allvars %>% 
  
  # Loop them through the 'or_tabulator' function we just created:
  ...
  # Select columns to drop for the final table:
  select(-c(or, lci95, uci95))


```

Finally, we can then append the odds ratios to the previous summary table and present the combined results in a `flextable` for easy comparison.  You can then consider what impact the two methods (crude prevalence ratios and odds ratios) have on the results, the width of the confidence intervals and what conclusions can be drawn from each.  

```{r s52_final_tab}

#################################################
# 03. Append crude odds ratios to the main table
#################################################

# Merge the two tables with a left join on variable and value columns:
restab52d <- restab52c %>% 
  
  # Append the odds ratio table to the counts/VCE/PR table:
  left_join(y = dplyr::select(.data = restab_or,
   ...
   # Recode missing values in the or95 column:
   mutate(or95 = case_when(is.na(or95) ~ "ref", # Recode missing values as ref 
                           TRUE ~ or95))        # If not missing, leave as is


#################################################
# 04. Create flextable of the combined results
#################################################

# Call up the data.frame:
restab52pub <- restab52d %>%
  ...
  flextable::set_header_labels(values = list(
    ...
    n_vce = "N vaccinated (% VCE)", 
    pr95 = "Prevalence rate (95% CI)", 
    or95 = "Crude odds ratio (95% CI)")) %>% 
  ...
  # Right-align last 3 columns (weighted VCE, PR and OR):
