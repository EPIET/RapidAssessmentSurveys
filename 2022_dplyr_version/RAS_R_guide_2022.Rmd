...
vaccine <- rio::import(here::here(file = "data", "vaccine4.dta"))  %>% 
  ...
  expss::apply_labels(vaccrec = "Response status", 
                      age = "Age in years", 
...
vc_total_03 <- round(x = (sum(strata_frame$pop_frac[1:3])
                      /sum(strata_frame$Total_pupils[1:3])) * 100, 
                     digits = 2)

# Calculate the total vaccination coverage estimate for all 11 strata:
vc_total_11 <- round(x = (sum(strata_frame$pop_frac)
                      /sum(strata_frame$Total_pupils)) * 100, 
                     digits = 2)
