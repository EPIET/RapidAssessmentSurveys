...
vaccine <- vaccine %>% 
  ...
  # Create a labelled factor from educf for father's education:
...
allvarlabs <- purrr::set_names(allvarlabs, allvars)
...
functions `tab_linelist()` to generate the counts for each
...
restab52 <- vaccine %>% 
  ...
  tab_linelist(allvars) %>% 
  
  # drop unweighted proportions variable 
  select(-proportion) %>% 
  
  # we can use our names vector of labels to recode the variable column
  # note the three exclamation marks (!) are scary advanced programming 
      # see the recode help file - but generally: just accept that it works
...
restab_vce <- purrr::map(
  ...
  .f = function(x) {
    ...
    vce_design %>% 
      ...
      # Add in the current variable name to make merging more accurate 
      mutate(variable = x)
  }
  ...
  dplyr::select(variable,                        # variable name from allvars
                value = stratum,                 # rename stratum to value (to match restab52)
...
restab52 <- restab52 %>% 
  ...
  left_join(y = dplyr::select(.data = restab_vce,
            ...
            # Indicate ID columns to join by 
            # (using both variable and value is safer
            # as yes/no could be falsely allocated):
            by = c("variable", "value")) %>%
...
stored in `coef.sd`. While there are helper functions (in the `broom`
package) that would do this heavy lifting for you with a standard `glm()`, 
this does not yet exist for `glmmML()` so we need to do it ourselves. 
...
counts and vaccine coverage, using the `variable` and `value` variables. We can then
add a 1 for the blanks, and convert the table to a `flextable` for
printing.

```{r s52_combined_results_table}

# Merge the two tables with a left join on variable and value:
restab52 <- restab52 %>% 
  ...
  # Encode intercepts (those with missing estimates) as "ref", 
  # leave all other rows unchanged:
  mutate(pr95 = case_when(
    pr95 == "NA (NA - NA)" ~ "ref", 
    TRUE                   ~ pr95
    )) %>% 
  
  # replace repeating row values(in variable names) with an empty string 
  # (this is so that they don't show up in the final table) 
  mutate(across(c(question, variable), ~replace(.x, duplicated(.x), ""))) %>% 
