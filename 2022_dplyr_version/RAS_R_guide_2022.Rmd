...
variable, by cross-tabulating `vacful` with the variable of interest
using the `janitor::tabyl()` function.
...
`sitrep::tab_survey()` as before. You can use the survey design that was
created in the previous task (`vce_design`).
...
vaccine <- vaccine %>% 
  ...
  mutate(educf = factor(educf, 
                           levels = c(2, 3, 4, 6),
                           labels = c("< 9 years (grammar school)", 
...
*B - Counts of full vaccinated children:*

In this section we will create a table of counts for vaccinated children by each of the variables listed in `allvars`:

```{r s52_counts}

counts <- vaccine %>% 
  #### Needs work
  # Add all variables sequentially to the table:
  janitor::tabyl(c(allvars), vacful)
