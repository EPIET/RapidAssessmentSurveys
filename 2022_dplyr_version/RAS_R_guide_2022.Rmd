...
include some multivariate modelling.
...
1.  Create an agegroup variable for mother's age (`mage` ) as follows:
    (\<25, 25-29, \>=30 years)
...
table below. Note that some will require labeling to make the output
more human readable.
...
| Father's education                         | `educf`      | Requires labeling (category) |
| Number of siblings                         | `nsibcat`    | Already a labeled factor     |
| Balanced positive opinion on vaccination   | `a1posyn`    | Requires labeling (binary)   |
| Balanced positive opinion on vaccination   | `a1posg`     | Requires labeling (category) |
| Uncritical positive opinion on vaccination | `uncritical` | Requires labeling (binary)   |
| Vaccination is necessary for my child      | `a1x1g`      | Requires labeling (binary)   |
| Vaccines may harm my child                 | `a1x2g`      | Requires labeling (binary)   |
| Natural disease preferable to vaccines     | `a1x5g`      | Requires labeling (binary)   |
| Long distance to immunization site         | `a3x1g`      | Requires labeling (binary)   |
| Inconvenient opening hours                 | `a3x2g`      | Requires labeling (binary)   |
| Cost of vaccines                           | `a3x5g`      | Requires labeling (binary)   |
...
`purrr::map()` to loop through the list of binary (0,1) variables and
...
vaccine <- vaccine %>%  
  ...
  # Mutate across:
  mutate(across(
    .cols = all_of(c(binary_vars)),   # Columns in the list binary_vars
...
# Create a liste of nice labels for allvars:
...
by each of the variables listed in `allvars`. We can use the `dplyr`
functions `pivot_longer()` and `count` to generate the counts for each
factor level in each variable from `allvars`.

We can then use `arrange` and `factor` to reorder the rows so that they
match the order of questions and responses in table 5.2. Note that for
each variable that is being summarized by `count()`, the factor levels
are already preserved.

Finally we will add a new column to the beginning of the table, to hold
human readable question labels that will make the final results table
...
restab52 <- vaccine %>% 
  ...
  tidyr::pivot_longer(cols = allvars,
                      names_to = "variable",
                      values_to = "response",
                      values_drop_na = TRUE) %>%
  
  # Add counts to the summary by variable and factor level:
  count(variable, response) %>% 
  
  
  # Now update order of the question groups to match table 5.2:
  dplyr::arrange(factor(x = variable, 
                        levels = allvars)) %>% 
  
  # Now create a new variable to hold the question labels:
  mutate(question = case_when(
    
    variable == "minority"    ~ "Minority group", 
    variable == "magegroup"   ~ "Mother's age at childbirth", 
    variable == "educf"       ~ "Father's education", 
    variable == "nsibcat"     ~ "Number of siblings", 
    variable == "a1posg"      ~ "Balanced positive opinion on vaccination", 
    variable == "a1posyn"     ~ "Balanced positive opinion on vaccination", 
    variable == "uncritical"  ~ "Uncritical positive opinion on vaccination", 
    variable == "a1x1g"       ~ "Vaccination is necessary for my child", 
    variable == "a1x2g"       ~ "Vaccines may harm my child", 
    variable == "a1x5g"       ~ "Natural disease preferable to vaccines", 
    variable == "a3x1g"       ~ "Long distance to immunization site", 
    variable == "a3x2g"       ~ "Inconvenient opening hours", 
    variable == "a3x5g"       ~ "Cost of vaccines"
    
    )) %>% 
  
  # Reorder columns so question labels come first:
  dplyr::relocate(question, .before = variable)
  
```

*C - Vaccination coverage stratified by parental characteristics and
beliefs:*
...
restab_vce <- purrr::map(
  ...
  .f = function(x) {
    ...
    vce_design %>% 
      sitrep::tab_survey(vacful,        # VCE will be calculated for vacful
                         strata = {x},  # each variable in allvars is a stratum
  ...
  dplyr::select(stratum,                         # stratum = response in restab52
                deff,                            # Design effect for reference
                n,                               # Estimated count for reference
                proportion,                      # VCE (%) to add to table
...
# Bind the two tables together on stratum = response:
restab52 <- restab52 %>% 
  
  # Join the vaccine coverage estimate to the counts by stratum / response:
  left_join(y = dplyr::select(.data = restab_vce,

                              # Columns from restab_vce to add to restab52:
                              stratum,
                              proportion),

            # Indicate ID columns to join by:
            by = c("response" = "stratum")) %>%
...
*D - Crude prevalence ratios for vaccination in association with
parental characteristics and beliefs:*
...
These need to be exponentiated to give the prevalence ratios. In
addition, you will also need to extract the standard errors, which are
stored in `coef.sd`

Note that a robust variance estimate is already made with the methods
used in the `glmmML` package, so it does not need to be specified.
Running this model as a standard `glm()` (available in base R) gives
less robust estimates.

The `glmmML()` function has a special argument for clustering, called
`cluster`. According to the study design, clustering was performed at
the level of school classes (`school`). However, the school class IDs
are numeric and therefore need to be converted to a factor before being
passed to the model (otherwise it would be treated like a continuous
variable).
...
# To tabulate results:
poisondf <- tibble(response = names(pmodel1$coefficients), # extract names too
                   pr = exp(pmodel1$coefficients), 
                   lci95 = exp(pmodel1$coefficients - 1.96 * pmodel1$coef.sd), 
                   uci95 = exp(pmodel1$coefficients + 1.96 * pmodel1$coef.sd))
...
poison_tabulator <- function(predictor) {
  ...
  modtab = tibble(
    ...
    # Separate variable name and response label into 2 columns:
...
Note that we removed the intercepts - in table 5.2, these have been
encoded as 1. We can now merge these results with the table containing
counts and vaccine coverage, using the `response` variable. We can then
add a 1 for the blanks, and convert the table to a `flextable` for
printing.

```{r s52_combined_results_table}

# Merge the two tables with a left join on response:
restab52 <- restab52 %>% 
  ...
  # Encode intercepts as 1:
  mutate(pr95 = case_when(pr95 == "NA (NA - NA)" ~ "1")) %>% 
...
the rest of the steps, as above. Remember to give the output a different
name, so that it does not get over-written. You can then append to the
above table and discuss any differences you observe in the results.
